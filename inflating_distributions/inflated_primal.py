...
class PrimalVertex:
    ...
    def add_measure_conservation_constraints_just_01(self, prog:MathematicalProgram, start_measure = None, target_measure = None):
    ...
    def add_measure_conservation_constraints(self, prog:MathematicalProgram, start_measure = None, target_measure = None, Q:npt.NDArray = np.zeros((3,3)) ):
        if len(self.edges_out) > 0:
...
class PrimalEdge:
    ...
    def get_right_measure(self, solution:MathematicalProgramResult = None):
        ls = 1+self.dim
        m = np.vstack( (np.hstack( (self.edge_measure[0,0], self.edge_measure[0, ls:]) ),
...
def plot_surface(fig:go.Figure, box:Hyperrectangle, m:npt.NDArray, name:str, scaling:float=1, cmax=10):
