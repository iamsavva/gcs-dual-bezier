...
def get_k_step_optimal_path(
    ...
    timer = timeit()
    solutions = solve_parallelized_convex_restriction(graph, vertex_paths, state, options, terminal_state=terminal_state, one_last_solve=False)
...
def lookahead_with_backtracking_policy(
    ...
    while not found_target:
        ...
        if decision_options[decision_index].empty():
            ...
            # timer = timeit()
            # solutions = solve_parallelized_convex_restriction(graph, vertex_paths, node.state_now, options, terminal_state=terminal_state, one_last_solve=False)
            # timer.dt("solving", print_stuff = options.verbose_solve_times)
            # num_times_solved_convex_restriction += 1
            # for (vertex_path, bezier_curves) in solutions:
            #     next_node = node.extend(bezier_curves[0], vertex_path[1])
    ...
    if found_target:
        full_path = postprocess_the_path(graph, target_node.vertex_path_so_far, target_node.bezier_path_so_far, initial_state, options, terminal_state)
...
def obtain_rollout(
    ...
    if options.use_lookahead_policy:
        rollout_path, v_path = lookahead_policy(graph, vertex, state, options, terminal_state)
    elif options.use_lookahead_with_backtracking_policy:
        rollout_path, v_path = lookahead_with_backtracking_policy(graph, vertex, state, options, terminal_state)
...
def get_statistics(
    ...
    rollout_path, rollout_v_path, rollout_dt = obtain_rollout(graph, lookahead, vertex, state, terminal_state)
