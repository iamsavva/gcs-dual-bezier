...
def random_uniform_graph_generator(
    ...
    if goal_uniform:
        ...
        for p in points:
            ...
            layer.append(v)
            vt = gcs.AddTargetVertexWithQuadraticTerminalCost(v_name + "T", box(p-0.1, p+0.1, 0, 0.1), Q_terminal, np.array([p,0]) )
        ...
        while x_now < x_max:
            ...
            layer.append(v)
            vt = gcs.AddTargetVertexWithQuadraticTerminalCost(v_name + "T", box(x_now-0.1, x_now+0.1, 0, 0.1), Q_terminal, np.array([x_now, 0]) )
            gcs.AddEdge(v, vt, cost_function)
            x_now += np.random.uniform(min_goal_blank, max_goal_blank, 1)[0]
            index += 1
    layers.append(layer)
