...
from bezier_dual_goal_conditioned import (
...
def random_uniform_graph_generator(
    ...
    layers = []
    # add first layer
    start_vertex = graph.AddVertex("0-0", box(x_min, x_max, num_layers - 1), vertex_is_start=True)
    ...
    for n in range(1, num_layers - 1):
        ...
        while x_now < x_max:
            ...
            if k % 2 == 0:
                ...
                v = graph.AddVertex(v_name, box(x_now, min(x_now + width, x_max), num_layers - 1 - n))
                layer.append(v)
                index += 1
                x_now += width
            k += 1
    ...
    if goal_uniform:
        ...
        for p in points:
            ...
            v = graph.AddVertex(v_name, box(p - target_set_width, p + target_set_width, 0))
            layer.append(v)
            vt = graph.AddTargetVertexWithQuadraticTerminalCost(
                v_name + "T",
                box(p - 0.1, p + 0.1, 0, 0.1),
                Q_terminal,
                np.array([p, 0]),
            )
            graph.AddEdge(v, vt, cost_function)
            index += 1
    else:
        x_now = np.random.uniform(min_goal_blank, max_goal_blank, 1)[0]
        while x_now < x_max:
            v_name = str(num_layers - 1) + "-" + str(index)
            v = graph.AddVertex(v_name, box(x_now - target_set_width, x_now + target_set_width, 0))
            layer.append(v)
            vt = graph.AddTargetVertexWithQuadraticTerminalCost(
                v_name + "T",
            ...
            for right_v in next_layer:
                if left_v.get_hpoly().IntersectsWith(right_v.get_hpoly()):
                    if use_bidirecitonal_edges:
                        graph.AddBidirectionalEdge(left_v, right_v, cost_function)
                    else:
                        graph.AddEdge(left_v, right_v, cost_function)

    # push up on start
    graph.MaxCostOverABox(start_vertex, start_vertex.convex_set.lb(), start_vertex.convex_set.ub())
    return graph, layers, start_vertex
...
def random_uniform_graph_generator_goal_conditioned(
    ...
    terminal_convex_set = Hyperrectangle([x_min,0], [x_max, 3])

    graph = GoalConditionedPolynomialDualGCS(options, terminal_convex_set)
    ...
    index = 0
    v_name = str(num_layers - 1) + "-" + str(index)
    v = graph.AddVertex(v_name, terminal_convex_set)
    graph.AddEdge(v, graph.terminal_vertex, cost_function)
    layer.append(v)
    layers.append(layer)
    ...
    # push up on start
    graph.MaxCostOverVertex(start_vertex)

    layers.append([graph.terminal_vertex])
    return graph, layers, start_vertex