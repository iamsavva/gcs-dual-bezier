...
def plot_bezier(fig:go.Figure, bezier_curves:T.List[T.List[npt.NDArray]], bezier_color="purple", control_point_color="red", name = None):
    showlegend = False
    line_name = ""
    if name is not None:
        line_name = name
    ...
    for curve_index, bezier_curve in enumerate(bezier_curves):
        ...
        if control_point_color is not None:
            fig.add_trace(go.Scatter(x=X[:,0], y=X[:,1], marker_color=control_point_color, marker_symbol='circle', mode = 'markers', showlegend=False))

        if curve_index == 0:
            fig.add_trace(go.Scatter(x=[xx[0,0]], y=[xx[0,1]], marker_color=bezier_color, mode = 'markers', showlegend=False))

    # plot bezier curves
    fig.add_trace(go.Scatter(x=full_path[:,0], y=full_path[:,1], marker_color=bezier_color, mode = 'lines', name=line_name, showlegend=showlegend))
...
def plot_a_2d_graph(vertices = T.List[DualVertex]):
    ...
    def add_trace(lb, ub):
        ...
        fig.add_trace(go.Scatter(x=xs, y=ys, line=dict(color="black"), fillcolor='grey', fill='tozeroy', showlegend=False ))
